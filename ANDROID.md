...
- Hub background music (`audio/bgm/music_*.mp3`) plays in the parent shell and ducks while a game iframe is open.
...
`ArcadeRouter.backToHub()` updates the UI and syncs browser history via `history.back()` — it does **not** navigate the iframe to `about:blank`, so iframe history stays clean.
